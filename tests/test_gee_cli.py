...
from ai_hydro.gee import cli


def test_status_json_output(capsys):
    with patch("ai_hydro.gee.cli.status") as mock_status:
...
def test_preview_chirps_mock(capsys):
    with patch("ai_hydro.gee.cli.preview_chirps_layer") as mock_preview:
...
def test_preview_layer_json_output(capsys):
    with patch("ai_hydro.gee.cli.preview_layer") as mock_preview:
        mock_preview.return_value = {
            "ok": True,
            "type": "gee_tile_layer",
            "name": "Layer",
...
def test_extract_timeseries_json_output(capsys):
    with patch("ai_hydro.gee.cli.extract_timeseries") as mock_extract:
